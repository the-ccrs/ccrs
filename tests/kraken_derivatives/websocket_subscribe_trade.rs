...
async fn main() {
    ...
    let kraken_derivatives_client = KrakenDerivativesClient::builder().build();

    let mut websocket_client = match kraken_derivatives_client
        .create_websocket_client(
            WebSocketClientConfig::kraken_derivatives_market_data(),
    ...
    subscribe_trade_request
        .symbols
        .push(get_env_as_string("SYMBOL", "PF_XBTUSD"));
