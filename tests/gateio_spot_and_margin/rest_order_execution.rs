...
async fn main() {
    ...
    let mut gateio_spot_and_margin_client_builder = GateioSpotAndMarginClient::builder();
    if get_env_as_bool("USE_TESTNET", false) {
        gateio_spot_and_margin_client_builder = gateio_spot_and_margin_client_builder
            .rest_api_base_url("https://api-testnet.gateapi.io");
    }

    let gateio_client = gateio_spot_and_margin_client_builder
