...
async fn main() {
    ...
    let htx_usdt_margined_futures_client = HtxUsdtMarginedFuturesClient::builder().build();

    let mut websocket_client = match htx_usdt_margined_futures_client
        .create_websocket_client(
            WebSocketClientConfig::htx_usdt_margined_futures_market_data(),
    ...
    subscribe_top_of_book_request
        .symbols
        .push(get_env_as_string("SYMBOL", "BTC-USDT"));
