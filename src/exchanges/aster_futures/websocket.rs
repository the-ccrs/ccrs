    ...
    for crate::exchanges::aster_futures::common::AsterFuturesClient
{
    fn websocket_api_url(&self, endpoint: crate::types::WebSocketEndpoint) -> String {
        match endpoint {
            crate::types::WebSocketEndpoint::AsterFutures(aster_endpoint) => match aster_endpoint {
                crate::types::AsterFuturesWebSocketEndpoint::MarketData => {
                    self.websocket_market_data_api_url.to_string()
                }
                crate::types::AsterFuturesWebSocketEndpoint::AccountData => {
                    self.websocket_account_data_api_url.to_string()
                }
                crate::types::AsterFuturesWebSocketEndpoint::Unknown => {
                    panic!("AsterFutures WebSocket endpoint is Unknown")
                }
            },
            _ => panic!("WebSocket endpoint is not AsterFutures"),
        }
    }
    ...
    async fn create_websocket_client(
        ...
        let websocket_client = match &websocket_client_config.endpoint {
            crate::types::WebSocketEndpoint::AsterFutures(
                crate::types::AsterFuturesWebSocketEndpoint::MarketData,
            ) => {
            ...
            crate::types::WebSocketEndpoint::AsterFutures(
...

                {
                    ...
                    let keepalive_client =
                        crate::exchanges::aster_futures::common::AsterFuturesClient::builder()
    ...
    fn is_websocket_text_trade_subscription_data(
        ...
        websocket_text
            .payload_summary
            .get("stream")
            .map(|stream| stream.ends_with("aggTrade"))
            .unwrap_or_default()
    }
    ...
    fn create_subscribe_top_of_book_websocket_subscription_data(
        ...
        let timestamp = crate::utils::convert_unix_timestamp_milliseconds_to_timestamp(
            data.get("E").and_then(|value| value.as_i64()).unwrap(),
        );
