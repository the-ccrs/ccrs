...
impl crate::exchange_client::websocket::Websocket
    for crate::exchanges::aster_spot::common::AsterSpotClient
{
    fn websocket_api_url(&self, endpoint: crate::types::WebSocketEndpoint) -> String {
        match endpoint {
            crate::types::WebSocketEndpoint::AsterSpot(aster_endpoint) => match aster_endpoint {
                crate::types::AsterSpotWebSocketEndpoint::MarketData => {
                    self.websocket_market_data_api_url.to_string()
                }
                crate::types::AsterSpotWebSocketEndpoint::AccountData => {
                    self.websocket_account_data_api_url.to_string()
                }
                crate::types::AsterSpotWebSocketEndpoint::Unknown => {
                    panic!("AsterSpot WebSocket endpoint is Unknown")
                }
            },
            _ => panic!("WebSocket endpoint is not AsterSpot"),
        }
    }
    ...
    async fn create_websocket_client(
        ...
        let websocket_client = match &websocket_client_config.endpoint {
            crate::types::WebSocketEndpoint::AsterSpot(
                crate::types::AsterSpotWebSocketEndpoint::MarketData,
            ) => {
            ...
            crate::types::WebSocketEndpoint::AsterSpot(
...

                {
                    ...
                    let keepalive_client =
                        crate::exchanges::aster_spot::common::AsterSpotClient::builder()
                        ...
                        loop {
                            tokio::select! {
                                _ = interval.tick() => {
                                    ...
                                    let now = chrono::Utc::now();
                                    crate::exchange_client::rest::Rest::sign_http_request(&keepalive_client, &mut http_request, now);
    ...
    fn populate_websocket_text_payload_summary(
        ...
        if let Some(json_payload) = &websocket_text.json_payload {
            if let Some(value) = json_payload.get("stream") {
            ...
            if let Some(value) = json_payload.get("e") {
                websocket_text.payload_summary.insert(
                    "e".to_string(),
                    value
                        .as_str()
            ...
            if let Some(value) = json_payload.get("status") {
                websocket_text.payload_summary.insert(
                    "status".to_string(),
                    value
                        .as_str()
            ...
            if let Some(value) = json_payload.get("result") {
                websocket_text
                    .payload_summary
                    .insert("result_null".to_string(), value.is_null().to_string());
            }

            if let Some(value) = json_payload.get("error") {
                websocket_text.payload_summary.insert(
                    "error".to_string(),
                    value
                        .as_str()
            ...
            if let Some(value) = json_payload.get("code") {
                websocket_text.payload_summary.insert(
                    "code".to_string(),
                    value
                        .as_str()
    ...
    fn create_subscribe_order_websocket_subscription_data(
        ...
        let order = self.convert_ws_execution_report_to_order(json_payload);
    ...
    fn create_subscribe_fill_websocket_subscription_data(
        ...
        crate::exchange_client::common::Response::WebSocketErrorResponse(websocket_text.clone())
    }
