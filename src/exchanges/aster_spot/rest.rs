...
impl crate::exchange_client::rest::Rest for crate::exchanges::aster_spot::common::AsterSpotClient {
    ...
    fn create_get_instrument_info_rest_response(
        ...
        for item in symbols {
            ...
            let max_notional = filters
                .iter()
                .find(|f| f["filterType"].as_str() == Some("MAX_NOTIONAL"));
            ...
            response.data.push(crate::types::InstrumentInfo {
                ...
                order_quantity_max: lot_size
                    ...
                    .unwrap_or_default()
                    .to_string(),
                order_quote_quantity_min: notional
                    .and_then(|f| f["minNotional"].as_str())
                    .or_else(|| min_notional.and_then(|f| f["minNotional"].as_str()))
                    .unwrap_or_default()
                    .to_string(),
                order_quote_quantity_max: notional
                    .and_then(|f| f["maxNotional"].as_str())
                    .or_else(|| max_notional.and_then(|f| f["maxNotional"].as_str()))
                    .unwrap_or_default()
                    .to_string(),
                ..Default::default()
            });
    ...
    fn create_get_top_of_book_rest_response(
        ...
        if json_payload.is_object() {
            let timestamp =
                chrono::DateTime::from_timestamp_millis(json_payload["time"].as_i64().unwrap())
                    .unwrap();
            ...
            for item in arr {
                let timestamp =
                    chrono::DateTime::from_timestamp_millis(item["time"].as_i64().unwrap())
                        .unwrap();
                response.data.push(crate::types::TopOfBook {
                    exchange_instrument_type: crate::types::ExchangeInstrumentType::AsterSpot,
                    symbol: item["symbol"].as_str().unwrap().to_string(),
