...
impl crate::exchange_client::websocket::Websocket
    for crate::exchanges::kalshi::common::KalshiClient
{
    ...
    fn websocket_api_url(&self, endpoint: crate::types::WebSocketEndpoint) -> String {
        match endpoint {
            crate::types::WebSocketEndpoint::Kalshi(_) => self.websocket_api_url.clone(),
            _ => panic!("Websocket endpoint is not Kalshi"),
        }
    }
    ...
    fn create_subscribe_top_of_book_websocket_subscription_data(
        ...
        let bid_price = msg["yes_bid_dollars"].as_str().unwrap_or("").to_string();
        let bid_size = msg["yes_bid_size_fp"].as_str().unwrap_or("").to_string();
        let ask_price = msg["yes_ask_dollars"].as_str().unwrap_or("").to_string();
        let ask_size = msg["yes_ask_size_fp"].as_str().unwrap_or("").to_string();
    ...
    fn create_subscribe_trade_websocket_subscription_data(
        ...
        let ts_ms = msg.get("ts_ms").and_then(|v| v.as_i64()).unwrap_or(0);
        let timestamp = crate::utils::convert_unix_timestamp_milliseconds_to_timestamp(ts_ms);
        let price = msg["yes_price_dollars"].as_str().unwrap_or("").to_string();
        let size = msg["count_fp"].as_str().unwrap_or("").to_string();
        let side = match msg.get("taker_side").and_then(|v| v.as_str()) {
            Some("yes") => crate::types::TakerSide::Buy,
            Some("no") => crate::types::TakerSide::Sell,
    ...
    fn create_subscribe_fill_websocket_subscription_data(
        ...
        let client_order_id = msg
            ...
            .unwrap_or("")
            .to_string();
        let side = match msg.get("action").and_then(|v| v.as_str()) {
            Some("buy") => crate::types::OrderSide::Buy,
            Some("sell") => crate::types::OrderSide::Sell,
            _ => crate::types::OrderSide::Unknown,
        };
        let price = msg["yes_price_dollars"].as_str().unwrap_or("").to_string();
        let quantity = msg["count_fp"].as_str().unwrap_or("").to_string();
        let is_maker = msg
            .get("is_taker")
